...
app = Flask(__name__)
def get_webcam(lat, lon):
    try:
        url = f"https://worldcam.eu/api/nearby?lat={lat}&lng={lon}"
        res = requests.get(url).json()
        webcams = res.get('result', {}).get('webcams', [])
        if webcams:
            # Return the embed link or direct URL
            return webcams[0].get('webcam_url')
        return None
